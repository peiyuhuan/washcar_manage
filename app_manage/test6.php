...
while ($row = mysql_fetch_array($data)) {
	...
	$newarray[]=array("id"=>$row['id'],"distance"=>$distance,"name"=>$row['name'],"address"=>$row['address'],"pic_url"=>$row['pic_url']);
	// $key++;
}

//多维数组排序
$arr2=multi_array_sort($newarray,"distance");

$arr3=array("user"=>$arr2);

$str=json_encode_ex($arr3);
